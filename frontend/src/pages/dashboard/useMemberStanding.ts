...
import { academicYear } from '../../lib/pointRequests';
    ...
    useEffect(() => {
        ...
        const unsubscribers = [
            onSnapshot(doc(db, 'users', memberEmail), (snap) => setMember((snap.data() as Member) ?? {}),
                (error) => console.error('Error loading member:', error)),
            ...
            onSnapshot(query(collection(db, 'pointRequests'), where('userEmail', '==', email)),
